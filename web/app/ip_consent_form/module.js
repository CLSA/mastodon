define( function() {
  ...
  try { var module = cenozoApp.module( 'ip_consent_form', true ); } catch( err ) { console.warn( err ); return; }
  ...
  cenozo.providers.directive( 'cnIpConsentFormList', [
    ...
    function( CnIpConsentFormModelFactory ) {
      return {
        ...
        scope: { model: '=?' },
        controller: function( $scope ) {
          if( angular.isUndefined( $scope.model ) ) $scope.model = CnIpConsentFormModelFactory.root;
        }
      };
    }
  ] );
  ...
  cenozo.providers.factory( 'CnIpConsentFormModelFactory', [
    'CnBaseFormModelFactory', 'CnIpConsentFormListFactory', 'CnIpConsentFormViewFactory',
    function( CnBaseFormModelFactory, CnIpConsentFormListFactory, CnIpConsentFormViewFactory ) {
