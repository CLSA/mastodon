...
cenozoApp.initFormModule = function (module, type) {
  ...
  if (angular.isDefined(module.actions.adjudicate)) {
    module.addExtraOperation("view", {
      ...
      operation: async function ($state, model) {
        await $state.go(type + "_form.adjudicate", $state.params);
      },
      isDisabled: function ($state, model) {
        return !model.viewModel.record.adjudicate;
      },
    });
  }
  ...
  module.addExtraOperation("view", {
    ...
    operation: async function ($state, model) {
      await $state.go("form.view", {
        identifier: model.viewModel.record.form_id,
      });
    },
    isIncluded: function ($state, model) {
      return model.viewModel.record.form_id;
    },
  });
};
...
cenozoApp.initFormEntryModule = function (module, type) {
  ...
  module.addExtraOperation("view", {
    ...
    operation: async function ($state, model) {
      await model.viewModel.downloadFile();
    },
  });
  ...
  module.addExtraOperation("list", {
    ...
    operation: async function ($state, model) {
      await model.listModel.startNewEntry();
    },
    isIncluded: function ($state, model) {
  ...
  module.addExtraOperation("view", {
    title: "Submit Entry",
    operation: async function ($state, model) {
      await model.viewModel.typistSubmitEntry();
    },
    isIncluded: function ($state, model) {
      return model.isRole("typist");
    },
  ...
  module.addExtraOperation("view", {
    ...
    isIncluded: function ($state, model) {
      return (
        !model.isRole("typist") && true === model.viewModel.record.submitted
      );
    },
  });
  module.addExtraOperation("view", {
    ...
    isIncluded: function ($state, model) {
      return (
        !model.isRole("typist") && false === model.viewModel.record.submitted
      );
    },
  });
};
    ...
    return {
      construct: function (object, module) {
        ...
        angular.extend(object, {
          ...
          view: async function (entryId) {
            await $state.go(formEntryName + ".view", { identifier: entryId });
          },
          ...
          onLoad: async function () {
            ...
            try {
                ...
                data: {
                  select: {
                    column: [
                      ...
                      "adjudicate",
                      {
                        column: validatedEntryColumn,
                        alias: "validated_form_id",
                      },
                    ],
                  },
                },
              ...
              object.conflictColumnList = object.formEntryList.reduce(function (
                list,
                entry,
                index
              ) {
                ...
                if (0 < index) {
                  for (var column in entry) {
                    ...
                    if (
                      object.formColumnList.findByProperty("column", column) &&
                      entry1 !== entry2
                    )
                      list.push(column);
                  }
    ...
    return {
      construct: function (object, module) {
        ...
        object.getEditEnabled = function () {
          return (
            object.$$getEditEnabled() && !object.viewModel.record.completed
          );
        };
      },
    };
  ...
  function (
    ...
    return {
      construct: function (object, parentModel) {
        ...
        var formType = formEntryName
          .substring(0, formEntryName.length - 11)
          .camelToSnake()
          .replace(/_/g, " ");
        ...
        object.startNewEntry = async function () {
              ...
              if (404 == error.status) {
                CnModalMessageFactory.instance({
                  ...
                  message:
                    "There are no new " +
                    formType +
                    " forms available for transcription at this time.",
                }).show();
              } else {
          ...
          await $state.go(formEntryName + ".view", {
      ...
      construct: function (object, parentModel, root) {
            ...
            onError: function (error) {
              if (400 == error.status && angular.isObject(error.data)) {
                CnModalMessageFactory.instance({
                  ...
                  message:
                    "There were errors found in the form which have been highlighted in red. " +
                ...
                for (var name in error.data) {
                  ...
                  if (element) {
                    if ("Cannot be blank." == error.data[name])
        ...
        object.typistSubmitEntry = async function () {
          var response = await CnModalConfirmFactory.instance({
            title: "Submit Entry",
            message:
              "Are you sure you wish to submit this form?  This should only be done after you have " +
      ...
      construct: function (object, module) {
        ...
        async function init() {
          ...
          if (object.isRole("typist")) {
            ...
            var mainInputGroup = module.inputGroupList.findByProperty(
              "title",
              ""
            );
            if (mainInputGroup) {
