...
CREATE PROCEDURE patch_consent_form_entry()
  BEGIN
    ...
    SELECT "Converting uid column to participant_id in consent_form_entry table" AS "";
    ...
    IF 1 = @test THEN
      ...
      SET @sql = CONCAT(
        ...
        "ADD CONSTRAINT fk_consent_form_entry_participant_id ",
      ...
      SET @sql = CONCAT(
        "UPDATE consent_form_entry ",
        "JOIN ", @cenozo, ".participant USING( uid ) ",
        "SET consent_form_entry.participant_id = participant.id"
      );
      PREPARE statement FROM @sql;
      EXECUTE statement;
      DEALLOCATE PREPARE statement;

      ALTER TABLE consent_form_entry DROP COLUMN uid;
    END IF;
