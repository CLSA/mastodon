...
CREATE PROCEDURE patch_proxy_form_entry()
  BEGIN
    ...
    SELECT "Converting uid column to participant_id in proxy_form_entry table" AS "";
    ...
    IF 1 = @test THEN
      ...
      SET @sql = CONCAT(
        ...
        "ADD CONSTRAINT fk_proxy_form_entry_participant_id ",
      ...
      SET @sql = CONCAT(
        "UPDATE proxy_form_entry ",
        "JOIN ", @cenozo, ".participant USING( uid ) ",
        "SET proxy_form_entry.participant_id = participant.id"
      );
      PREPARE statement FROM @sql;
      EXECUTE statement;
      DEALLOCATE PREPARE statement;

      ALTER TABLE proxy_form_entry DROP COLUMN uid;
    END IF;
