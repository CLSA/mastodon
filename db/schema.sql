...
-- Schema mastodon
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `mastodon` DEFAULT CHARACTER SET utf8 COLLATE utf8_bin ;
USE `cenozo` ;
...
CREATE TABLE IF NOT EXISTS `mastodon`.`dm_consent_form` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_form_id` (`form_id` ASC),
  INDEX `fk_validated_dm_consent_form_entry_id` (`validated_dm_consent_form_entry_id` ASC),
  CONSTRAINT `fk_dm_consent_form_form_id`
    FOREIGN KEY (`form_id`)
    REFERENCES `cenozo`.`form` (`id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `fk_dm_consent_form_validated_dm_consent_form_entry_id`
...
CREATE TABLE IF NOT EXISTS `mastodon`.`dm_consent_form_entry` (
  ...
  `user_id` INT(10) UNSIGNED NOT NULL,
  `submitted` VARCHAR(45) NOT NULL DEFAULT 0,
  `participant_id` INT(10) UNSIGNED NULL DEFAULT NULL,
  `accept` VARCHAR(45) NOT NULL DEFAULT 0,
  `alternate_id` INT(10) UNSIGNED NULL DEFAULT NULL,
  `signed` VARCHAR(45) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `mastodon`.`ip_consent_form` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_form_id` (`form_id` ASC),
  INDEX `fk_validated_ip_consent_form_entry_id` (`validated_ip_consent_form_entry_id` ASC),
  CONSTRAINT `fk_ip_consent_form_form_id`
    FOREIGN KEY (`form_id`)
    REFERENCES `cenozo`.`form` (`id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `fk_ip_consent_form_validated_ip_consent_form_entry_id`
...
BEGIN
  ...
  SET dm_consent_form_id = proc_dm_consent_form_id,
      entry_total = (
...
BEGIN
  ...
  SET ip_consent_form_id = proc_ip_consent_form_id,
      entry_total = (
...
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`dm_consent_form_AFTER_INSERT` AFTER INSERT ON `dm_consent_form` FOR EACH ROW
BEGIN
  CALL update_dm_consent_form_total( NEW.id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`dm_consent_form_entry_AFTER_INSERT` AFTER INSERT ON `dm_consent_form_entry` FOR EACH ROW
BEGIN
  CALL update_dm_consent_form_total( NEW.dm_consent_form_id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`dm_consent_form_entry_AFTER_UPDATE` AFTER UPDATE ON `dm_consent_form_entry` FOR EACH ROW
BEGIN
  CALL update_dm_consent_form_total( NEW.dm_consent_form_id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`dm_consent_form_entry_AFTER_DELETE` AFTER DELETE ON `dm_consent_form_entry` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`ip_consent_form_AFTER_INSERT` AFTER INSERT ON `ip_consent_form` FOR EACH ROW
BEGIN
  CALL update_ip_consent_form_total( NEW.id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`ip_consent_form_entry_AFTER_INSERT` AFTER INSERT ON `ip_consent_form_entry` FOR EACH ROW
BEGIN
  CALL update_ip_consent_form_total( NEW.ip_consent_form_id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`ip_consent_form_entry_AFTER_UPDATE` AFTER UPDATE ON `ip_consent_form_entry` FOR EACH ROW
BEGIN
  CALL update_ip_consent_form_total( NEW.ip_consent_form_id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`ip_consent_form_entry_AFTER_DELETE` AFTER DELETE ON `ip_consent_form_entry` FOR EACH ROW
BEGIN
