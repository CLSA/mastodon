...
-- -----------------------------------------------------
-- Table `mastodon`.`proxy_consent_form`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mastodon`.`proxy_consent_form` (
  ...
  CONSTRAINT `fk_proxy_consent_form_form_id`
    ...
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `fk_proxy_consent_form_proxy_consent_form_entry_id`
    FOREIGN KEY (`validated_proxy_consent_form_entry_id`)
    REFERENCES `mastodon`.`proxy_consent_form_entry` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `mastodon`.`proxy_consent_form_entry`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mastodon`.`proxy_consent_form_entry` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL,
  `create_timestamp` TIMESTAMP NOT NULL,
  ...
  CONSTRAINT `fk_proxy_consent_form_entry_proxy_consent_form_id`
    ...
    REFERENCES `mastodon`.`proxy_consent_form` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
-- -----------------------------------------------------
-- Table `mastodon`.`proxy_consent_form_total`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mastodon`.`proxy_consent_form_total` (
  `proxy_consent_form_id` INT UNSIGNED NOT NULL,
  `update_timestamp` TIMESTAMP NOT NULL,
  `create_timestamp` TIMESTAMP NOT NULL,
  `entry_total` INT(11) NOT NULL,
...
BEGIN
  ...
  SET proxy_consent_form_id = proc_proxy_consent_form_id,
      ...
      cohort = (
        SELECT GROUP_CONCAT( DISTINCT cohort.name ORDER BY cohort.name SEPARATOR ',' )
        FROM proxy_consent_form_entry
        LEFT JOIN cenozo.participant ON proxy_consent_form_entry.uid = participant.uid
        LEFT JOIN cenozo.cohort ON participant.cohort_id = cohort.id
        WHERE proxy_consent_form_id = proc_proxy_consent_form_id
        GROUP BY proxy_consent_form_id
      );
...
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`proxy_consent_form_AFTER_INSERT` AFTER INSERT ON `proxy_consent_form` FOR EACH ROW
BEGIN
  CALL update_proxy_consent_form_total( NEW.id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`proxy_consent_form_entry_AFTER_INSERT` AFTER INSERT ON `proxy_consent_form_entry` FOR EACH ROW
BEGIN
  CALL update_proxy_consent_form_total( NEW.proxy_consent_form_id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`proxy_consent_form_entry_AFTER_UPDATE` AFTER UPDATE ON `proxy_consent_form_entry` FOR EACH ROW
BEGIN
  CALL update_proxy_consent_form_total( NEW.proxy_consent_form_id );
END$$

USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`proxy_consent_form_entry_AFTER_DELETE` AFTER DELETE ON `proxy_consent_form_entry` FOR EACH ROW
BEGIN
