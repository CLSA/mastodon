...
-- Thu 23 Jan 2020 02:54:44 PM EST
...
CREATE TABLE IF NOT EXISTS `mastodon`.`consent_form_entry` (
  ...
  INDEX `dk_uid` (`uid` ASC),
  INDEX `fk_consent_form_entry_user_id_idx` (`user_id` ASC),
...
CREATE TABLE IF NOT EXISTS `mastodon`.`proxy_form_entry` (
  ...
  INDEX `dk_uid` (`uid` ASC),
  INDEX `fk_proxy_form_entry_user_id_idx` (`user_id` ASC),
  INDEX `fk_proxy_form_entry_proxy_region_id_idx` (`proxy_region_id` ASC),
  INDEX `fk_proxy_form_entry_informant_region_id_idx` (`informant_region_id` ASC),
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mastodon`.`general_proxy_form_entry` ;
...
CREATE TABLE IF NOT EXISTS `mastodon`.`general_proxy_form_entry` (
  ...
  `informant_note` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uq_general_proxy_form_id_user_id` (`general_proxy_form_id` ASC, `user_id` ASC),
  INDEX `dk_uid` (`uid` ASC),
  INDEX `fk_user_id` (`user_id` ASC),
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mastodon`.`general_proxy_form` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mastodon`.`general_proxy_form_total` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mastodon`.`extended_hin_form_entry` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mastodon`.`extended_hin_form` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mastodon`.`extended_hin_form_total` ;
...
USE `mastodon`;
DROP procedure IF EXISTS `mastodon`.`update_general_proxy_form_total`;
...
USE `mastodon`;
DROP procedure IF EXISTS `mastodon`.`update_extended_hin_form_total`;
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`hin_form_entry_AFTER_INSERT` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`hin_form_entry_AFTER_INSERT` AFTER INSERT ON `hin_form_entry` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`hin_form_entry_AFTER_UPDATE` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`hin_form_entry_AFTER_UPDATE` AFTER UPDATE ON `hin_form_entry` FOR EACH ROW
BEGIN
  CALL update_hin_form_total( NEW.hin_form_id );
END$$
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`hin_form_entry_AFTER_DELETE` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`hin_form_entry_AFTER_DELETE` AFTER DELETE ON `hin_form_entry` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`hin_form_AFTER_INSERT` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`hin_form_AFTER_INSERT` AFTER INSERT ON `hin_form` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`general_proxy_form_entry_AFTER_INSERT` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`general_proxy_form_entry_AFTER_INSERT` AFTER INSERT ON `general_proxy_form_entry` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`general_proxy_form_entry_AFTER_UPDATE` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`general_proxy_form_entry_AFTER_UPDATE` AFTER UPDATE ON `general_proxy_form_entry` FOR EACH ROW
BEGIN
  CALL update_general_proxy_form_total( NEW.general_proxy_form_id );
END;$$
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`general_proxy_form_entry_AFTER_DELETE` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`general_proxy_form_entry_AFTER_DELETE` AFTER DELETE ON `general_proxy_form_entry` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`general_proxy_form_AFTER_INSERT` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`general_proxy_form_AFTER_INSERT` AFTER INSERT ON `general_proxy_form` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`extended_hin_form_entry_AFTER_INSERT` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`extended_hin_form_entry_AFTER_INSERT` AFTER INSERT ON `extended_hin_form_entry` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`extended_hin_form_entry_AFTER_UPDATE` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`extended_hin_form_entry_AFTER_UPDATE` AFTER UPDATE ON `extended_hin_form_entry` FOR EACH ROW
BEGIN
  CALL update_extended_hin_form_total( NEW.extended_hin_form_id );
END$$
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`extended_hin_form_entry_AFTER_DELETE` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`extended_hin_form_entry_AFTER_DELETE` AFTER DELETE ON `extended_hin_form_entry` FOR EACH ROW
BEGIN
...
USE `mastodon`$$
DROP TRIGGER IF EXISTS `mastodon`.`extended_hin_form_AFTER_INSERT` $$
USE `mastodon`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mastodon`.`extended_hin_form_AFTER_INSERT` AFTER INSERT ON `extended_hin_form` FOR EACH ROW
BEGIN
