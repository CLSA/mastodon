...
final class log extends singleton
{
  ...
  private function send( $message, $type )
  {
    ...
    if( !class_exists( 'mastodon\business\session' ) || !business\session::exists() ) return;
...
set_error_handler( array( '\mastodon\log', 'error_handler' ) );
register_shutdown_function( array( '\mastodon\log', 'fatal_error_handler' ) );
?>
