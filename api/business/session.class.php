...
final class session extends \cenozo\business\session
{
   ...
   * This method overrides the parent method since sites have a application as well as a name
   * 
   * @author Patrick Emond <emondpd@mcmaster.ca>
  ...
  protected function process_requested_site_and_role( $site_name, $role_name )
  {
    ...
    {
      // make sure there is an instance name in the header
      if( !array_key_exists( 'HTTP_INSTANCE', $_SERVER ) )
        throw lib::create( 'exception\runtime',
          'Instance name missing, unable to process requested site and role', __METHOD__ );

      $application_class_name = lib::get_class_name( 'database\application' );
      $site_class_name = lib::get_class_name( 'database\site' );
      $db_application =
        $application_class_name::get_unique_record( 'name', $_SERVER['HTTP_INSTANCE'] );
      $this->requested_site = $site_class_name::get_unique_record(
        array( 'application_id', 'name' ),
        array( $db_application->id, $site_name ) );
