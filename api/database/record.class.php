 ...
 * @package mastodon\database
 */
abstract class record extends \mastodon\base_object
{
  /**
   * Constructor
   * 
    ...
    else if( 'get' == $action )
    {
      ...
      { // calling one of: get_<record>_list( $modifier = NULL )
        ...
        if( 0 < count( $args ) &&
            !is_null( $args[0] ) &&
            is_object( $args[0] ) &&
            'mastodon\\database\\modifier' != get_class( $args[0] ) )
  ...
  protected function get_record( $record_type )
  {
    ...
    {
      $class_name = '\\mastodon\\database\\'.$record_type;
    ...
    $record_type, $modifier = NULL, $inverted = false, $count = false )
  {
    ...
    $foreign_class_name = '\\mastodon\\database\\'.$record_type;
    ...
    if( !is_null( $modifier ) )
    {
      ...
      foreach( $tables as $table )
      {
        if( $table && 0 < strlen( $table ) && $table != $this_table )
        {
          ...
          if( static::column_exists( $foreign_key_name ) )
          {
            $class_name = '\\mastodon\\database\\'.$table;
